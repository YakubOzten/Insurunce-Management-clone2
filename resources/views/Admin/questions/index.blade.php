    ...
    <div class="card">
        ...
        <table class="table-responsive">
            ...
            @foreach($quests as $quest)
                <tr>
                    <input type="hidden" class="questdelete_val_id" value="{{$quest->id}}">
...
{{--                    <td>{{ $product->status == '0'? "Visible":"Hidden"   }} </td>--}}
                    ...
                    <td style="display:flex">
                            ...
                            <button type="button" class="btn btn-primary" data-toggle="modal"
                                    data-target="#qeditModal{{$quest->id}}"><i class="fa-regular fa-message-plus">Teklifi EKLE</i>

                            </button>
                        <button class="btn btn-danger questdeletebtn"> <i class="fa fa-trash"></i>SİL</button>
{{--                        <form action="{{route('yonetici-catdelete',$category->id)}}" method="post">--}}
{{--                            {{method_field('DELETE')}}--}}
{{--                            {{csrf_field()}}--}}
{{--                            <button class="btn btn-danger " type="submit">Sil</button>--}}
{{--                        </form>--}}
                        <div class="modal fade bd-example-modal-lg" id="qeditModal{{$quest->id}}" tabindex="-1"
...
@section('scripts')
    <script>
        $(document).ready(function () {
            $('.questdeletebtn').click(function (e) {
                e.preventDefault();
                var delete_id = $(this).closest("tr").find('.questdelete_val_id').val();
